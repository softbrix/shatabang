...
var httpClient = axios.create({baseUrl: BASE_URL, withCredentials: true});
axios.defaults.baseURL = BASE_URL;


function deleteImages(imageList) {
  httpClient.post('/api/images/delete', imageList).then(function(response) {
...
(function($) {
  // TODO: Display error
  httpClient.get('/api/account').then(function(response) {
    if(response.status === 200 && response.data.user) {
      ...
      httpClient.get('/api/auth/list').then(function(response) {
  ...
  var loadImageList = function(folder) {
    return httpClient.get('/images/info/'+folder+'/media.lst')
  ...
  httpClient.get('/api/dirs/list').then(function(response) {
  ...
  setInterval(function() {
    httpClient.get('/api/upload/imported')
      .then(function (response) {
